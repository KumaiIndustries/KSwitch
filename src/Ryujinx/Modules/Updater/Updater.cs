...
namespace Ryujinx.Modules
{
    internal static class Updater
    {
        ...
        public static async Task BeginParse(Window mainWindow, bool showVersionUpToDate)
        {
            ...
            {
                Logger.Error?.Print(LogClass.Application, "Failed to convert the current KSwitch version!");
            ...
            if (_updateSuccessful)
            {
                ...
                if (shouldRestart)
                {
                    ...
                    else
                    {
                        ...
                        {
                            ryuName = OperatingSystem.IsWindows() ? "KSwitch.exe" : "KSwitch";
                        }
...
#pragma warning restore SYSLIB0014
        ...
        public static void CleanupUpdate()
        {
            ...
            if (!OperatingSystem.IsMacOS())
            {
                ...
                string currentRyuName = Path.GetFileName(Environment.ProcessPath) ?? (OperatingSystem.IsWindows() ? "KSwitch.exe" : "KSwitch");
